...
test('ACTUAL-STATUS-GUARD-02: 完了中end clear禁止、解除後有効、進行中から未着手を確認',async({page})=>{
  ...
  await page.locator('.ganttRow[data-task-id="D"] .actualEditBtn').click();await expect(page.locator('#actualEndClear')).toBeDisabled();await page.locator('#actualEditBack button').filter({hasText:'キャンセル'}).click();await page.evaluate(()=>{window.confirm=()=>true;changeState(data.items.findIndex(x=>x.id==='D'),'進行中')});expect(await page.evaluate(()=>itemById('D').actual_end)).toBe('2026-08-02');await page.locator('.ganttRow[data-task-id="D"] .actualEditBtn').click();await expect(page.locator('#actualEndClear')).toBeEnabled();await page.locator('#actualEditBack button').filter({hasText:'キャンセル'}).click();
