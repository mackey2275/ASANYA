...
async function boot(page){
  ...
  await page.reload();
  await page.evaluate(async()=>await applyJsonObject({schema_version:CURRENT_SCHEMA_VERSION,workspace_info_markdown:'',items:[],snapshots:[]},'Playwright','phase3.json',null,{remember:false,writePermissionGranted:false}));
}
async function setData(page,items){
  await page.evaluate(async items=>await applyJsonObject({schema_version:CURRENT_SCHEMA_VERSION,workspace_info_markdown:'',items,snapshots:[]},'Playwright','phase3.json',null,{remember:false,writePermissionGranted:false}),items);
}
