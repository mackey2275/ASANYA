...
test('GANTT-CHILD-02: task-add menu child action uses common draft path; Esc and blank Enter leave no provisional task',async({page})=>{
  await setData(page,[task('parent','親')]);await gantt(page);const before=await page.evaluate(()=>data.items.length);await page.locator('.ganttRow[data-task-id="parent"] .taskAddBtn').click();await page.getByRole('menuitem',{name:'1つ下の階層に追加'}).click();expect(await page.evaluate(()=>draftKind)).toBe('child');await page.locator('#ganttView .ganttDraftTitle').press('Escape');expect(await page.evaluate(()=>({count:data.items.length,draft:draftTaskId,saved:persistableData().items.length}))).toEqual({count:before,draft:'',saved:before});await page.locator('.ganttRow[data-task-id="parent"] .taskAddBtn').click();await page.getByRole('menuitem',{name:'1つ下の階層に追加'}).click();await page.locator('#ganttView .ganttDraftTitle').press('Enter');expect(await page.evaluate(()=>({count:data.items.length,draft:draftTaskId}))).toEqual({count:before,draft:''});
});

test('GANTT-CHILD-03: 編集中Insertは誤発火せず、新規child計画設定でbar・sort・summaryを更新',async({page})=>{
  await setData(page,[task('parent','親'),task('late','遅い子',{parentId:'parent',due:'2026-08-20',planned_duration_days:1,sortOrder:2000})]);await gantt(page);await page.locator('.ganttRow[data-task-id="late"] .ganttPlanned').click();let input=page.locator('.ganttRow[data-task-id="late"] .ganttPlanned input');await expect(input).toBeFocused();await input.press('Insert');expect(await page.evaluate(()=>draftTaskId)).toBe('');await input.press('Escape');await page.locator('.ganttRow[data-task-id="parent"] .taskAddBtn').click();await page.getByRole('menuitem',{name:'1つ下の階層に追加'}).click();const id=await page.evaluate(()=>draftTaskId);await finishDraft(page,'早い子');await page.locator(`.ganttRow[data-task-id="${id}"] .ganttPlanned`).click();input=page.locator(`.ganttRow[data-task-id="${id}"] .ganttPlanned input`);await input.fill('1');await input.press('Enter');await page.locator(`.ganttRow[data-task-id="${id}"] .ganttDue .dueTxt`).click();input=page.locator(`.ganttRow[data-task-id="${id}"] .ganttDueText`);await input.fill('2026-08-05');await input.press('Enter');await expect(page.locator(`.ganttBar[data-task-id="${id}"]`)).toBeVisible();expect((await ganttOrder(page)).slice(0,3)).toEqual(['parent',id,'late']);await expect(page.locator('.ganttBar.summary[data-task-id="parent"]')).toHaveAttribute('data-start','2026-08-05');
});

test('GANTT-CHILD-04: childは親終了をblockするが親の状態変更はblockしない',async({page})=>{
  await setData(page,[task('parent','親'),task('child','子',{parentId:'parent'})]);await gantt(page);await page.locator('.ganttRow[data-task-id="parent"] .ganttState select').selectOption('進行中');expect(await page.evaluate(()=>itemById('parent').state)).toBe('進行中');await page.locator('.ganttRow[data-task-id="parent"] .ganttState select').selectOption('完了');expect(await page.evaluate(()=>itemById('parent').state)).toBe('完了');const pending=page.waitForEvent('dialog'),action=page.locator('.ganttRow[data-task-id="parent"] .doneBtn').click(),dialog=await pending;expect(dialog.message()).toContain('子');await dialog.accept();await action;expect(await page.evaluate(()=>itemById('parent').completed)).toBe(false);
});
