...
test('PHASE4-ROLL-06 ToDo/Project rolloverはDue FLIPを使いProject dock位置を維持',async({page})=>{
  const items=[task('R',{due:'2026-08-01',repeat:'毎月',recurrence_rule:{type:'monthly',mode:'date',day:1}}),...Array.from({length:35},(_,i)=>task('T'+i,{due:`2026-08-${String(Math.min(28,i+2)).padStart(2,'0')}`,sortOrder:(i+2)*1000}))];await boot(page,items,'team');await expect(page.locator('.projectOuterScrollDock')).toBeVisible();const outer=page.locator('.projectOuterScrollDock');await outer.evaluate(el=>{el.scrollLeft=Math.min(600,el.scrollWidth-el.clientWidth);el.dispatchEvent(new Event('scroll'))});const before=await page.evaluate(()=>({outer:ganttView.scrollLeft,anchor:rcCaptureViewport().anchorDay,pageY:scrollY}));await page.locator('.ganttRow[data-task-id="R"] .ganttState select').selectOption('完了');await expect(page.locator('.ganttRow[data-task-id="R"]')).toHaveAttribute('data-sort-move-kind','due');await expect.poll(()=>page.locator('.sortMoveAnimating').count(),{timeout:4000}).toBe(0);const after=await page.evaluate(()=>({outer:ganttView.scrollLeft,anchor:rcCaptureViewport().anchorDay}));expect(after.outer).toBe(before.outer);expect(Math.abs(after.anchor-before.anchor)).toBeLessThan(.01);expect(await value(page,'R')).toMatchObject({id:'R',due:'2026-09-01',completed:false,state:''})
});
