...
test('GANTT-ACTUAL-UI-01: date入力検証・user化・保留タスクの正順クリア',async({page})=>{
  await show(page,[task('edit','編集',{state:'保留',due:'2026-08-20',planned_duration_days:5})]);await expect(page.locator('.actualEditBtn')).toHaveCount(0);await page.evaluate(()=>openTaskDetailPane('edit'));const pane=page.locator('#taskDetailPane');const start=pane.getByRole('textbox',{name:'実績開始',exact:true}),end=pane.getByRole('textbox',{name:'実績終了',exact:true});await start.fill('2026/08/02');await start.press('Enter');await end.fill('2026/08/04');await end.press('Enter');expect(await page.evaluate(()=>itemById('edit'))).toMatchObject({actual_start_source:'user',actual_end_source:'user',due:'2026-08-20',planned_duration_days:5});await end.fill('');await end.press('Enter');await start.fill('');await start.press('Enter');const x=await page.evaluate(()=>itemById('edit'));expect(x).not.toHaveProperty('actual_start');expect(x).not.toHaveProperty('actual_start_source');expect(x).not.toHaveProperty('actual_end');expect(x).not.toHaveProperty('actual_end_source');
});
