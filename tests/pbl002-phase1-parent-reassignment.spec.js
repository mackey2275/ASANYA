...
async function boot(page,items,mode='personal',workspace='PBL情報'){await page.goto(APP);await page.evaluate(()=>localStorage.clear());await page.reload();await page.evaluate(async({items,mode,workspace})=>{await applyJsonObject({schema_version:CURRENT_SCHEMA_VERSION,workspace_info_markdown:workspace,items,...(CURRENT_SCHEMA_VERSION==='3.2'?{snapshots:[]}:{})},'PBL','pbl.json',null,{remember:false,writePermissionGranted:false});setView('all');setMode(mode);clearUndoHistory('pbl')},{items,mode,workspace})}
...
test('PBL1-02 child/root/grandchildの各方向をSchema変更なしで移動',async({page})=>{
  ...
  expect(await page.evaluate(()=>({parent:itemById('A').parentId,schema:data.schema_version}))).toEqual({parent:'C',schema:await page.evaluate(()=>CURRENT_SCHEMA_VERSION)});
});
...
test('PBL1-09 保存・再読込でparent/order/subtree/Schema/workspace情報を保持',async({page})=>{
  await boot(page,[task('A'),task('B',{parentId:'A'}),task('C',{parentId:'B'}),task('D')],'personal','# 重要情報');await page.evaluate(()=>reassignTaskParent('B','D'));const json=await page.evaluate(()=>JSON.stringify(persistableData()));await page.evaluate(async json=>await applyJsonObject(JSON.parse(json),'reload','pbl.json',null,{remember:false,writePermissionGranted:false}),json);
  expect(await page.evaluate(()=>({schema:data.schema_version,workspace:data.workspace_info_markdown,parent:itemById('B').parentId,order:itemById('B').sortOrder,child:itemById('C').parentId}))).toEqual({schema:await page.evaluate(()=>CURRENT_SCHEMA_VERSION),workspace:'# 重要情報',parent:'D',order:1000,child:'B'});
});
