...
test('PBL1-02 child/root/grandchildの各方向をSchema変更なしで移動',async({page})=>{
  ...
  expect(await page.evaluate(()=>({parent:itemById('A').parentId,schema:data.schema_version}))).toEqual({parent:'C',schema:'3.1'});
});
...
test('PBL1-09 保存・再読込でparent/order/subtree/Schema/workspace情報を保持',async({page})=>{
  ...
  expect(await page.evaluate(()=>({schema:data.schema_version,workspace:data.workspace_info_markdown,parent:itemById('B').parentId,order:itemById('B').sortOrder,child:itemById('C').parentId}))).toEqual({schema:'3.1',workspace:'# 重要情報',parent:'D',order:1000,child:'B'});
});
