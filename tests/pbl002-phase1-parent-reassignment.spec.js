...
async function boot(page,items,mode='personal',workspace='PBL情報'){await page.goto(APP);await page.evaluate(()=>localStorage.clear());await page.reload();await page.evaluate(({items,mode,workspace})=>{applyJsonObject({schema_version:'2.2',workspace_info_markdown:workspace,items},'PBL','pbl.json',null,{remember:false,writePermissionGranted:false});setView('all');setMode(mode);clearUndoHistory('pbl')},{items,mode,workspace})}
...
test('PBL1-02 child/root/grandchildの各方向をSchema変更なしで移動',async({page})=>{
  ...
  expect(await page.evaluate(()=>({parent:itemById('A').parentId,schema:data.schema_version}))).toEqual({parent:'C',schema:'2.2'});
});
...
test('PBL1-09 保存・再読込でparent/order/subtree/Schema/workspace情報を保持',async({page})=>{
  ...
  expect(await page.evaluate(()=>({schema:data.schema_version,workspace:data.workspace_info_markdown,parent:itemById('B').parentId,order:itemById('B').sortOrder,child:itemById('C').parentId}))).toEqual({schema:'2.2',workspace:'# 重要情報',parent:'D',order:1000,child:'B'});
});
