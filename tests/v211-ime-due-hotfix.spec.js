...
test('V211-IME-B: full-width composition no-sort waits for compositionend and commits once',async({page})=>{
  ...
  expect(await page.evaluate(()=>({due:itemById('TARGET').due,trace:__imeHotfixTrace,pending:dueImeShortcutPending&&{digit:dueImeShortcutPending.digit,finishingRequested:dueImeShortcutPending.finishingRequested}}))).toEqual({due:before.due,trace:{save:0,render:0},pending:{digit:1,finishingRequested:true}});
