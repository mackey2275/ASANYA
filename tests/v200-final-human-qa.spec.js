...
test('SCHEMA-200-02/03/04 1.9 opens unchanged, real save emits 2.0, future rejects',async({page})=>{
  ...
  expect(await page.evaluate(()=>({loadedSchemaVersion,schemaMigrationPending,dataSchema:data.schema_version}))).toEqual({loadedSchemaVersion:'1.9',schemaMigrationPending:true,dataSchema:'2.0'});
  const persisted=await page.evaluate(()=>persistableData()); expect(persisted.schema_version).toBe('2.0'); expect(persisted.items[0]).toMatchObject({id:'A',actual_start_source:'user',recurrence_rule:{type:'daily'}});
  const before=await page.evaluate(()=>JSON.stringify(data.items)); const result=await page.evaluate(()=>{try{prepareSchemaObject({schema_version:'2.1',items:[]});return'accepted'}catch(e){return e.schemaKind}}); expect(result).toBe('newer'); expect(await page.evaluate(()=>JSON.stringify(data.items))).toBe(before);
});
...
test('SCHEMA-200-01/02/03 File API mock proves create 2.0 and open-only does not write',async({page})=>{
  ...
  await expect.poll(()=>page.evaluate(()=>currentDbName)).toBe('New.json');const created=await page.evaluate(()=>__fsMock.directoryEntries('newdir')[0][1]);expect(JSON.parse(await page.evaluate(id=>__fsMock.snapshot(id).text,created))).toEqual({schema_version:'2.0',items:[]});
