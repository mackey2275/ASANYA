...
test('SCHEMA-200-01/02/03 File API mock proves current Schema create and legacy open-only does not write',async({page})=>{
  ...
  await expect.poll(()=>page.evaluate(()=>currentDbName)).toBe('New.json');const created=await page.evaluate(()=>__fsMock.directoryEntries('newdir')[0][1]),current=await page.evaluate(()=>CURRENT_SCHEMA_VERSION);expect(JSON.parse(await page.evaluate(id=>__fsMock.snapshot(id).text,created))).toEqual(current==='3.2'?{schema_version:'3.2',workspace_info_markdown:'',items:[],snapshots:[]}:current==='3.1'?{schema_version:'3.1',workspace_info_markdown:'',items:[]}:current==='3.0'?{schema_version:'3.0',workspace_info_markdown:'',items:[]}:current==='2.5'?{schema_version:'2.5',workspace_info_markdown:'',items:[]}:current==='2.2'?{schema_version:'2.2',workspace_info_markdown:'',items:[]}:{schema_version:'2.0',items:[]});
  ...
  await page.evaluate(()=>chg(data.items.findIndex(x=>x.id==='L'),'owner','edited'));if(['2.5','3.0','3.1','3.2'].includes(current)){expect(await page.evaluate(()=>({dirty,schemaMigrationPending,writes:__fsMock.snapshot('legacy').writeCount}))).toEqual({dirty:true,schemaMigrationPending:true,writes:0})}else{await page.evaluate(()=>performDbSave({allowDownload:false,source:'test'}));const saved=JSON.parse(await page.evaluate(()=>__fsMock.snapshot('legacy').text));expect(saved.schema_version).toBe(await page.evaluate(()=>CURRENT_SCHEMA_VERSION));expect(saved.items[0]).toMatchObject({id:'L',title:'legacy',owner:'edited',sortOrder:1000})}
});
