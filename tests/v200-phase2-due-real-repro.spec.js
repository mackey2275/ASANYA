...
await fresh(page,mode);await page.evaluate(()=>{itemById('TARGET').due=addDays(ymd(),7);render()});const row=target(page,mode);await row.evaluate(el=>{el.scrollIntoView({block:'center'});const r=el.getBoundingClientRect(),s=taskVerticalScroller(el),p=s===document.scrollingElement||s===document.documentElement||s===document.body,b=rcUsableVerticalBounds(s,p),delta=r.bottom-(b.bottom-4);if(p)window.scrollBy({top:delta,behavior:'auto'});else s.scrollTop+=delta});const before=await page.evaluate(()=>scrollY);await row.locator('.dueTxt').click();const input=row.locator('input[type="text"][id^="d"]');await input.press('d');await input.press('9');await expect(row).toHaveClass(/sortMoveAnimating/);await expect.poll(()=>page.locator('.sortMoveAnimating').count(),{timeout:4000}).toBe(0);const after=await page.evaluate(()=>scrollY),final=await finalPosition(row),usableCenter=(final.usableTop+final.usableBottom)/2;expect(after-before).toBeGreaterThan(5);expect(after-before).toBeLessThan((final.usableBottom-final.usableTop)*.5);expect(final.top).toBeGreaterThanOrEqual(final.usableTop-1);expect(final.bottom).toBeLessThanOrEqual(final.usableBottom+1);expect(Math.abs(final.center-usableCenter)).toBeGreaterThan((final.usableBottom-final.usableTop)*.1)
});
