...
test('UNDO-03: 未確定フィールドのCtrl+Zは編集開始値へ戻し履歴を消費しない',async({page})=>{
  await boot(page,[task('A','Alpha',{due:'2026-08-20'})]);await page.locator('#mPersonal').click();
...
test('UNDO-03A: child draftは1回目field、2回目draft、3回目確定履歴の順にUndo',async({page})=>{
  ...
  await page.locator('#mTeam').click();await page.evaluate(()=>selectTask('A'));await page.keyboard.press('Enter');const id=await page.evaluate(()=>draftTaskId);await page.locator(`#row_${id} .titleText`).fill('Draft');await page.locator(`#row_${id} .titleText`).press('Enter');const due=page.locator(`#row_${id} input[id^="d"]`);await due.fill('2026-08-25');await due.press('Control+z');
...
test('UNDO-03B: 期限手入力・期限なしはCtrl+Z一回で開始値へ戻り通常表示',async({page})=>{
  await boot(page,[task('A','With due',{due:'2026-08-20'}),task('B','No due',{sortOrder:2000})]);await page.locator('#mPersonal').click();
...
test('UNDO-03C: date picker clear相当の未確定空値も開始期限へ戻してeditorを閉じる',async({page})=>{
  await boot(page,[task('A','Alpha',{due:'2026-08-20'})]);await page.locator('#mTeam').click();await page.evaluate(()=>setMode('team'));await page.locator('.ganttRow[data-task-id="A"] .ganttDue .dueTxt').click();const input=page.locator('.ganttRow[data-task-id="A"] .ganttDueText');await input.fill('');await page.locator('.ganttRow[data-task-id="A"] .ganttDueCalendar').evaluate(el=>{el.value='';el.dispatchEvent(new Event('change',{bubbles:true}))});await input.press('Control+z');expect(await page.evaluate(()=>itemById('A').due)).toBe('2026-08-20');await expect(page.locator('.ganttRow[data-task-id="A"] .ganttDueText')).toBeHidden();await expect(page.locator('.ganttRow[data-task-id="A"] .ganttDue')).toContainText(await page.evaluate(()=>dueLab('2026-08-20')));expect(await page.evaluate(()=>undoStack.length)).toBe(0);
});

test('UNDO-03D: d→1は通常状態・期限editor経由とも確定しUndo/Redo可能',async({page})=>{
  await boot(page,[task('A','Alpha',{due:'2026-08-20'})]);await page.locator('#mPersonal').click();await page.evaluate(()=>selectTask('A'));const shortcutDue=await page.evaluate(()=>addDays(ymd(),1));
...
test('UNDO-03F: composition中Ctrl+Zはapp処理せず、その後field→draft→historyの順を維持',async({page})=>{
  await boot(page,[task('P','Parent'),task('B','Existing child',{parentId:'P',owner:'before',sortOrder:2000})]);await page.evaluate(()=>chg(1,'owner','confirmed'));await page.waitForTimeout(0);await page.locator('#mTeam').click();await page.evaluate(()=>selectTask('B'));await page.keyboard.press('Enter');const id=await page.evaluate(()=>draftTaskId),title=page.locator(`#row_${id} .titleText`);await title.dispatchEvent('compositionstart',{data:'日'});await title.evaluate(el=>{el.textContent='日本';el.dispatchEvent(new InputEvent('input',{bubbles:true,data:'日本',inputType:'insertCompositionText'}));el.dispatchEvent(new KeyboardEvent('keydown',{key:'z',ctrlKey:true,isComposing:true,keyCode:229,bubbles:true,cancelable:true}))});await title.dispatchEvent('compositionend',{data:'日本'});
...
test('UNDO-03E: Project List新規追加行はsort移動先へfollow・赤attentionし作成履歴は1件',async({page})=>{
  await boot(page,[task('E','Early',{due:'2026-08-10',planned_duration_days:1}),task('L','Late',{due:'2026-08-30',planned_duration_days:1,sortOrder:2000})]);await page.locator('#mTeam').click();
...
test('UNDO-07: due marker dragは1履歴でUndo/Redoし選択を追従',async({page})=>{
  await boot(page,[task('A','Alpha',{due:'2026-08-20'})]);await page.locator('#mTeam').click();await page.evaluate(()=>setMode('team'));const marker=page.locator('.ganttRow[data-task-id="A"] .ganttDueOnlyMarker');await marker.scrollIntoViewIfNeeded();const box=await marker.boundingBox();await page.mouse.move(box.x+2,box.y+2);await page.mouse.down();await page.mouse.move(box.x+56,box.y+2,{steps:8});await page.mouse.up();await page.waitForTimeout(0);const moved=await page.evaluate(()=>itemById('A').due);expect(moved).not.toBe('2026-08-20');expect(await page.evaluate(()=>undoStack.length)).toBe(1);
