...
test('UNDO-04: Projectドラフトは完了全体が1履歴、取消は履歴なし',async({page})=>{
  ...
  await page.keyboard.press('Enter');let id=await page.evaluate(()=>draftTaskId);await page.locator(`#row_${id} .titleText`).fill('Created');await page.locator(`#row_${id} .titleText`).press('Enter');await page.locator(`#row_${id} input[id^="d"]`).fill('2026-08-25');await page.locator(`#row_${id} input[id^="d"]`).press('Enter');await page.waitForTimeout(0);
...
test('UNDO-07: due marker dragは1履歴でUndo/Redoし選択を追従',async({page})=>{
  await boot(page,[task('A','Alpha')]);const original=await page.evaluate(()=>{const x=itemById('A');x.due=addDays(ymd(),3);clearUndoHistory('undo-07-fixture');dirty=false;return x.due});await page.locator('#dmProjectDetail').click();await page.evaluate(()=>setMode('team'));await expect.poll(()=>page.evaluate(()=>hqa2TimelineInitialized)).toBe(true);const marker=page.locator('.ganttRow[data-task-id="A"] .ganttDueOnlyMarker');await marker.evaluate(el=>{const clip=document.querySelector('.ganttHeader .ganttTimelineClip'),left=parseFloat(el.style.left)||0;setGanttTimelineScroll(Math.max(0,left-clip.clientWidth/2))});await marker.scrollIntoViewIfNeeded();await page.evaluate(()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))));const box=await marker.boundingBox(),start={x:box.x+box.width/2,y:box.y+box.height/2};expect(await page.evaluate(start=>document.elementFromPoint(start.x,start.y)?.closest?.('.ganttDueOnlyMarker')?.classList.contains('ganttDueOnlyMarker')===true,start)).toBe(true);const dayWidth=await page.evaluate(()=>GANTT_DAY_WIDTH);await page.mouse.move(start.x,start.y);await page.mouse.down();await page.mouse.move(start.x-dayWidth,start.y,{steps:8});await page.mouse.up();await page.waitForTimeout(0);const moved=await page.evaluate(()=>itemById('A').due);expect(moved).not.toBe(original);expect(await page.evaluate(()=>undoStack.length)).toBe(1);
  await page.keyboard.press('Control+z');expect(await page.evaluate(()=>({due:itemById('A').due,selected:selectedTaskId}))).toEqual({due:original,selected:'A'});await page.keyboard.press('Control+y');expect(await page.evaluate(()=>itemById('A').due)).toBe(moved);
});
