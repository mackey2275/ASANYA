...
test('TOP-01: Start/上部では非表示、ToDo・Project List・Ganttで表示してsmoothに先頭へ戻る',async({page})=>{
  ...
  for(const openView of [
    async()=>page.locator('#mPersonal').click(),
    async()=>{await page.locator('#mTeam').click();await page.evaluate(()=>setMode('team'))},
    async()=>{await page.locator('#mTeam').click();await page.evaluate(()=>setMode('team'))}
  ]){
...
test('TOP-02: Gantt dock・toastと重ならず、横scroll/resizeや履歴・保存状態へ影響しない',async({page})=>{
  await boot(page);await page.locator('#mTeam').click();await page.evaluate(()=>setMode('team'));await page.locator('.ganttRow[data-task-id="TOP-60"]').scrollIntoViewIfNeeded();await page.evaluate(()=>syncUnifiedDockGeometry());
...
test('SUMMARY-HOVER-01: ToDo/Project List概要hoverはtipを出さずGantt tooltipは維持',async({page})=>{
  ...
  for(const mode of ['#mPersonal','#mTeam']){await page.locator(mode).click();const summary=page.locator('#row_TOP-1 .sum');await summary.hover();await page.waitForTimeout(30);await expect(tip).toBeHidden();expect(await summary.getAttribute('onmouseenter')).toBeNull();expect(await summary.getAttribute('onmousemove')).toBeNull()}
