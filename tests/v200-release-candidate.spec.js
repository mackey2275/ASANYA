...
const {APP,APP_FS_PATH,TARGET_PRODUCT_VERSION,TARGET_SCHEMA_VERSION}=require('./helpers/app-target');
const isV310=TARGET_SCHEMA_VERSION==='3.1'||APP.includes('pbl034'),isV300=APP.includes('v300')||APP.includes('pbl022')||APP.includes('pbl018'),isV250=(APP.includes('v250')||APP.includes('v260')||APP.includes('v270')||APP.includes('v300')),isV240=APP.includes('v240'),isV230=APP.includes('v230')||APP.includes('task_detail_phase'),isPbl002=APP.includes('pbl002_'),expectedSchema=TARGET_SCHEMA_VERSION||(isV310?'3.1':isV300?'3.0':isV250?'2.5':APP.includes('v220')||isPbl002||isV230||isV240?'2.2':'2.0'),expectedProduct=TARGET_PRODUCT_VERSION?'ASANYA v'+TARGET_PRODUCT_VERSION:isV310?'ASANYA v3.1.0':APP.includes('v300')?'ASANYA v3.0.0':APP.includes('v270')?'ASANYA v2.7.0':APP.includes('v260')?'ASANYA v2.6.0':isV250?'ASANYA v2.5.0':isV240?'ASANYA v2.4.0':isV230?'ASANYA v2.3.0':APP.includes('v220_dev')?'ASANYA v2.2.0-dev':APP.includes('v220')||isPbl002?'ASANYA v2.2.0':APP.includes('v211_dev')?'ASANYA v2.1.1-dev':APP.includes('v211')?'ASANYA v2.1.1':APP.includes('v210')?'ASANYA v2.1.0':'ASANYA v2.0.0';
