...
async function boot(page){
  ...
  await expect(page).toHaveTitle(TARGET_PRODUCT_VERSION?'ASANYA v'+TARGET_PRODUCT_VERSION:APP.includes('pbl034')?'ASANYA v3.1.0':APP.includes('v270')?'ASANYA v2.7.0':APP.includes('v260')?'ASANYA v2.6.0':'ASANYA v2.5.0');expect(await page.evaluate(()=>CURRENT_SCHEMA_VERSION)).toBe(CURRENT_SCHEMA);
}
