...
const json=(page,items)=>page.evaluate(items=>JSON.stringify({schema_version:CURRENT_SCHEMA_VERSION,...(CURRENT_SCHEMA_VERSION!=='1.5'?{workspace_info_markdown:''}:{}),items,...(CURRENT_SCHEMA_VERSION==='3.2'?{snapshots:[]}:{})}),items);
...
async function boot(page){
  ...
  await page.evaluate(async()=>await applyJsonObject({schema_version:CURRENT_SCHEMA_VERSION,workspace_info_markdown:'',items:[],snapshots:[]},'Playwright','phase3b.json',null,{remember:false,writePermissionGranted:false}));
}
async function setData(page,items){await page.evaluate(async items=>await applyJsonObject({schema_version:CURRENT_SCHEMA_VERSION,workspace_info_markdown:'',items,snapshots:[]},'Playwright','phase3b.json',null,{remember:false,writePermissionGranted:false}),items);}
