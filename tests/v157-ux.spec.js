...
expect(await page.evaluate(id=>JSON.parse(__fsMock.snapshot(id).text),newId)).toEqual(app.includes('v220_dev')||app.includes('pbl002_')||app.includes('v230')?{schema_version:'2.2',workspace_info_markdown:'',items:[]}:{schema_version:'2.0',items:[]});await expect(page.locator('#dbReadBtn')).toHaveText('別DB読込');await expect(page.locator('#dbSaveBtn')).toHaveText('現在DBのコピーを保存');await expect(page.locator('#newDbBtn')).toBeVisible();
});
